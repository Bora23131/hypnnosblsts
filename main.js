...
const settings = {
    "clientID": "botunuzunid",
    "clientSecret": "clientscreetburaya",
    "callbackurl": "https://domAin/auth/callback"
};

app.get("/robots.txt", function(req,res) {
    res.set('Content-Type', 'text/plain');
    res.send(`User-agent: *\nAllow: /\nSitemap: https://SunLight/sitemap.xml`);
});

app.get("/sitemap.xml", function(req,res) {
    let link = "<url><loc>https://discordlist.fun/</loc></url>";
    Object.values(db.get("bots")).forEach(bot => {
        link += "\n<url><loc>https://discordlist.fun/bot/"+bot.id+"</loc></url>";
    })
...
app.get("/discord", function (req,res) {
    res.redirect("https://discord.gg/75ZUHEKQrM");
});

app.get("/devils", function (req,res) {
    res.redirect("https://discord.gg/8XwTZnUzkY");
});
...
passport.serializeUser((user, done) => {
 ...
 passport.deserializeUser((obj, done) => {
   ...
   app.get("/login", async (req, res, next) => {
    ...
    request({
        url: `https://discordapp.com/api/v8/guilds/766340643617767485/members/${req.user.id}`,
        method: "PUT",
        json: {
    ...
    if (db.get(`users.${req.user.id}.ban`) === true) return res.json({login: false, message: "Oops!! You are banned from SunLight."});
    ...
    console.log("SunLight is opening...");
