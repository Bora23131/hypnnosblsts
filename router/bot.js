...
router.get("/bot/:id", async (req,res) => {
    ...
    if (data.status === 0) {
        ...
        let member = client.guilds.cache.get("766340643617767485").members.cache.get(req.user.id)
        if (member){
    ...
    let badge = false;
    if (client.guilds.cache.get("766340643617767485").members.cache.get(getbotdata.id)) {
       badge = client.guilds.cache.get("766340643617767485").members.cache.get(getbotdata.id).roles.cache.get("794266963282886686")
    }
    res.render("bot.ejs", {
