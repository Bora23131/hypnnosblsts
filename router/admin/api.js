...
router.post("/posts/approve", async (req,res) => {
    ...
    try {
        client.guilds.cache.get("766340643617767485").member(db.get(`bots.${data.id}`).owner).roles.add("788517922967388171");        
    } catch (error) {
    ...
    if (owner) {
        owner.send(`:tada: Your bot named **${db.get(`bots.${id}`).username}** has been approved! Your bot's page: https://discordlist.fun/bot/${id}\n\n:tada: **${db.get(`bots.${id}`).username}** adlı botun onaylandı! |  https://discordlist.fun/bot/${id}  `);
        try {
            client.channels.cache.get("795594275680550923").send(`<:sunlight_pinkonay:801410269552836659> ・ <@${db.get(`bots.${id}`).owner}>'s bot named **${db.get(`bots.${id}`).username}** was approved by <@${req.user.id}>.\n\n:flag_tr: ・ <@${db.get(`bots.${id}`).owner}> , **${db.get(`bots.${id}`).username}** adlı botun <@${req.user.id}> tarafından onaylandı!`);
        } catch (error) {
            console.log(error)
        }
    }
...
router.post("/posts/decline", (req,res) => {
    ...
    if (!member) return;
    if (!member.roles.cache.get("788517834116169779")) return;
    ...
    if (owner) {
        try {
            owner.send(`<:sunlight_pinkred:801410334870863903> ・ Your bot named **${db.get(`bots.${id}`).username}** has been declined! \n\nReason: \`\`\` ${data.reason}\`\`\` `);
        } catch (error) {
            console.log(error);
        }
        client.channels.cache.get("805862362128187414").send(`<:sunlight_pinkred:801410334870863903> ・ <@${db.get(`bots.${id}`).owner}>'s bot named **${db.get(`bots.${id}`).username}** was declined by <@${req.user.id}>.\n\n:flag_tr: ・ <@${db.get(`bots.${id}`).owner}> , **${db.get(`bots.${id}`).username}** adlı botun <@${req.user.id}> tarafından reddedildi.`);
    }

    if (data.action == "kick") {
        try {
            if(client.guilds.cache.get("766340643617767485").members.cache.get(`${id}`))
                client.guilds.cache.get("766340643617767485").members.cache.get(`${id}`).kick();
        } catch (error) {
            console.log(error)
        }
    }
...
router.post("/posts/ban", async(req,res) => {
    ...
    let banMember = client.guilds.cache.get("766340643617767485").members.cache.get(`${id}`);
    if (!banMember) {
    ...
    if (Object.values(db.get(`bots`)).filter(d => d.owner == banMember || d.owners.includes(banMember))) {
        for (let i = 0; i < Object.values(db.get(`bots`)).filter(d => d.owner == banMember || d.owners.includes(banMember))[0].length; i++) {
            ...
            try {
                await client.guilds.cache.get("766340643617767485").members.cache.get(`${memberbots.id}`).kick();
             } catch (error) {
                 console.log(error)
             }
        }
    ...
    try {
       await client.guilds.cache.get("766340643617767485").members.cache.get(`${id}`).ban();
    } catch (error) {
        console.log(error)
    }
...
router.post("/posts/give-premium", (req,res) =>{
    ...
    try {
        client.guilds.cache.get("766340643617767485").member(data.id).roles.add("801411074586443796");        
    } catch (error) {
...
router.post("/posts/remove-premium", (req,res) =>{
    ...
    try {
        client.guilds.cache.get("766340643617767485").member(data.id).roles.remove("801411074586443796");        
    } catch (error) {
